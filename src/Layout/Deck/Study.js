...
function Study() {
  ...
  const flip = (e) => {
    if (deck.cards) {
      if (cardText === deck.cards[cardNum - 1].front) {
        ...
        if (cardNum !== cardTotal) setOnBack((bool) => (bool = true));
        else setOnEnd((bool) => (bool = true));
      } else {
  ...
  useEffect(() => {
    if (deck.cards) {
      ...
      if (deck.cards[cardNum - 1]) setCardText(deck.cards[cardNum - 1].front);
      else setCardText("");
    }
  }, [deck, cardNum]);
    ...
    return (
      <>
        ...
        {cardTotal > 2 ? (
          ...
          <NotEnoughCards cardLength={cardTotal} />
        )}
      </>
    );
  else return null;
